...
def add_watermark(image_path, watermark_text):
    ...
    image.convert('RGB').save('watermarked_image.jpg')
...
def add_watermark2():
    ...
    while True:
        ...
        x_pos = frame_width - wm_width - 500
        y_pos = frame_height - wm_height - 500
...
if __name__ == '__main__':
    print("Test- Watermark Program!!!")
    while True:
        otp = int(input('Lựa chọn:'))
        if otp == 0: break
        if otp == 1: add_watermark('C:\\Users\\admin\\Pictures\\Images\\gray.jpg', 'WATERMARK');
        if otp == 2: add_watermark2()
        if otp == 3: add_watermark3()